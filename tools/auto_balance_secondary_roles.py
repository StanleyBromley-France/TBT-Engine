...
from balancing_scripts.secondary_roles.scoring import compute_secondary_role_score
...
class SecondaryRoleCandidateEvaluator:
    def __init__(
        ...
        self._cache: dict[tuple[int, int, int, int, int], measurements.SecondaryRoleMeasurement] = {}

    def evaluate(self, candidate: tuple[int, int, int, int, int]) -> measurements.SecondaryRoleMeasurement:
...
def prepare_eval_content(config: config_models.SecondaryRoleBalancerConfig) -> Path:
...
def load_initial_candidate(config: config_models.SecondaryRoleBalancerConfig, content_path: Path) -> tuple[int, int, int, int, int]:
    unit_templates = scenarios.load_unit_templates(content_path)
    matched_units = [
        unit_template
        for unit_template in unit_templates
        if unit_template.get("secondaryRole") == config.balance.target_secondary_role
        and (
            config.balance.target_primary_role is None
            or unit_template.get("primaryRole") == config.balance.target_primary_role
        )
    ]
    if not matched_units:
        raise ValueError(
            "No units matched "
            f"secondaryRole={config.balance.target_secondary_role!r}, "
            f"primaryRole={config.balance.target_primary_role!r}."
        )
...
def build_initial_population(
    ...
    seed_candidates = [
        initial_candidate,
        (initial_candidate[0], config.balance.unit_max_mana_points_max, initial_candidate[2], initial_candidate[3], initial_candidate[4]),
        (initial_candidate[0], config.balance.unit_max_mana_points_min, initial_candidate[2], initial_candidate[3], initial_candidate[4]),
    ]
    population: list = []
...
def mutate_candidate(individual: list[int], config: config_models.SecondaryRoleBalancerConfig, rng: random.Random):
    ranges = [
        (config.balance.unit_max_hp_min, config.balance.unit_max_hp_max, 14),
        (config.balance.unit_max_mana_points_min, config.balance.unit_max_mana_points_max, 10),
        (config.balance.unit_move_points_min, config.balance.unit_move_points_max, 1),
        (config.balance.physical_damage_received_percent_min, config.balance.physical_damage_received_percent_max, 8),
        (config.balance.magic_damage_received_percent_min, config.balance.magic_damage_received_percent_max, 8),
    ]
...
def run_secondary_role_ga(
    config: config_models.SecondaryRoleBalancerConfig,
    candidate_evaluator: Callable[[tuple[int, int, int, int, int]], measurements.SecondaryRoleMeasurement],
    initial_candidate: tuple[int, int, int, int, int],
) -> measurements.SecondaryRoleMeasurement:
    from deap import base, creator, tools

    if not hasattr(creator, "SecondaryRoleFitnessMax"):
        creator.create("SecondaryRoleFitnessMax", base.Fitness, weights=(1.0,))
    if not hasattr(creator, "SecondaryRoleIndividual"):
        creator.create("SecondaryRoleIndividual", list, fitness=creator.SecondaryRoleFitnessMax)
    ...
    measurement_cache: dict[tuple[int, int, int, int, int], measurements.SecondaryRoleMeasurement] = {}
    ...
    population = build_initial_population(config, initial_candidate, rng, creator.SecondaryRoleIndividual)
...
def print_candidate_measurement(
    ...
    print(
        "candidate "
        f"secondary-role={config.balance.target_secondary_role} primary-role={primary_filter} "
        ...
        f"secondary-role-score={measurement.secondary_role_alignment_score:.4f} "
...
def optimize_secondary_role(
    ...
    candidate_evaluator = SecondaryRoleCandidateEvaluator(config, content_path, eval_config, offensive_ability_ids)

    best_measurement = run_secondary_role_ga(config, candidate_evaluator.evaluate, initial_candidate)
...
def run(config: config_models.SecondaryRoleBalancerConfig) -> int:
    ...
    print(
        ...
        f"secondary-role-score={best_measurement.secondary_role_alignment_score:.4f} "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Tune unit template stats for one secondary role.")
